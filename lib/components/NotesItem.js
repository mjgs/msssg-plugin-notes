...
class NotesItem {
  ...
  getItemUrl(webRoot, mergedItems) {
    const currentWebRoot = (mergedItems) ? 
      this.item?.itemMeta?.customComponentEnvProps?.webRoot :
      webRoot;

    debug(`getItemUrl - currentWebRoot: [${currentWebRoot}]`);
    
    const url = `${currentWebRoot}/${this.item.itemDatePath}/${this.item.itemSlug}`;
  ...
  getTemplate() {
    return `
      ...
      <% if (dateTimePrefixSpecified) { %><%= prefix %><% } %> <%- mdToHtml(item.body) %> <a href="<%= getItemUrl(webRoot, mergedItems) %>">#</a>
    `;
  }
}
