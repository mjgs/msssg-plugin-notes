...
class Notes {
  ...
  getTemplate() {
    return `
      ...
      <% const dateTimePrefixEnabled = (typeof dateTimePrefix !== 'undefined') ? dateTimePrefix : true; %>
      
      <% const postsByCalendarItemComponentEnvProps = { itemTitleHtmlTag, listClass, author, webRoot, postsByCalendar, dateTimePrefix: dateTimePrefixEnabled }; %>
      
      <%- await renderComponent(new components.PostsByCalendar(dictionaryName, 'sortedByDateDaily', webRoot, perPage, postsByCalendarItemComponentName, postsByCalendarItemComponentEnvProps, perDateItemOrdering), { itemTitleHtmlTag, listClass }); %>
      ...
      <ul class="<%= listClass %>">
        ...
        <li>
          <article>
            <p>
              <%- await renderComponent(new components.NotesItem(item), { itemTitleHtmlTag, listClass, webRoot, postsByCalendar }); %>
            </p>
          </article>
        </li>
