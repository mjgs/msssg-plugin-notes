...
class Notes {
  ...
  getTemplate() {
    return `
      <article> 
        <%- await renderComponent(new components.PostsByCalendar(dictionaryName, webRoot, postsPerPage, perDateItemOrdering), { postsByCalendarItemComponent: 'NotesItem', listClass }); %>
      </article>
    `;
  }
}
