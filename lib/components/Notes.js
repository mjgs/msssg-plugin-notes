...
constructor(dictionaryName, sortedBy, author, webRoot, perPage, perDateItemOrdering = 'reverse-chronological', dateTimePrefix = false, itemTitleHtmlTag = 'h2', postsListClass = 'bulletless', itemsListClass = 'bulletless') {
  ...
  getTemplate() {
    return `
      ...
      <%- await renderComponent(new components.Posts(dictionaryName, sortedBy, author, webRoot, perPage, undefined, postsListClass, itemsListClass, postItemComponentName, postItemComponentEnvProps), { perDateItemOrdering }); %>
    `;
  }
}
